(function(){
  ...
  .controller('MainController', [
    'navService', '$mdSidenav', '$mdBottomSheet', '$log', '$q', '$state', '$mdToast', '$http', '$scope', '$interval',
    MainController
  ]);

  function MainController(navService, $mdSidenav, $mdBottomSheet, $log, $q, $state, $mdToast, $http, $scope, $interval) {
    var vm = this;
...
/*
    ...
    vm.data = [
      { key: "GET", values: [ [1025409600000, 5.1], [ 1025409600020, 3.2 ]] }
    ]
*/
    vm.apis = [];
    vm.stats = [];

    $http.get( 'http://127.0.0.1:4000/apis' ).then( function( response ) {
      vm.apis = response.data.data;
    })

    $scope.data = [ { key: 'GET', values: [] }, { key: 'POST', values: [] }]
    ...
    setInterval( function() {
      ...
      for( i in vm.apis ) {
        var a = vm.apis[i];
        $http.get( 'http://127.0.0.1:4000/apis/' + a.id + '/stats' ).then( function( response ) {
          _values = response.data.results[0].series[0].values.map( function( e ) {
            return [new Date(e[0]), e[1]]
          })

          _postvalues = response.data.results[0].series[1].values.map( function( e ) {
            return [new Date(e[0]), e[1]]
          })

          // $scope.data[0].values.push([Date.now(),Math.random()*5])
          $scope.data[0].values =  _values

          $scope.data[1].values =  _postvalues

          if ($scope.data[0].values.length > 20) $scope.data[0].values.shift();
          if ($scope.data[1].values.length > 20) $scope.data[1].values.shift();
    ...
    setInterval( function() {
      ...
      for( i in vm.apis ) {
        ...
        $http.get( 'http://127.0.0.1:4000/apis/' + a.id + '/stats' ).then( function( response ) {
          ...
          vm.data[0].values = response.data.results[0].series[0].values.map( function( e ) {
            return [new Date(e[0]), e[1]]
          })

          vm.data[1].values = response.data.results[0].series[1].values.map( function( e ) {
            return [new Date(e[0]), e[1]]
          })

          vm.apiStats.updateWithData(vm.data)
          vm.apiStats.refresh()
      ...
      // vm.data[0].values.push( [ new Date(), 1.4] )
